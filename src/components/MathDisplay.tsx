...
const MathDisplay: React.FC<Props> = ({ n, t, iso, ev }) => {
  ...
  const calcStr = `EV = ${aperturePart.toFixed(2)} ${shutterPart >= 0 ? '+' : ''}${shutterPart.toFixed(2)} ${isoPart >= 0 ? '+' : ''}${isoPart.toFixed(2)} = ${ev.toFixed(2)}`;

  return (
    <div className="panel math-display">
      <h2>Real-time Math Engine</h2>
      ...
      <div className="offset-explanation">
        ...
        <p>
          Aperture contribution ($2\log_2(N)$): <span className="highlight">{aperturePart.toFixed(2)}</span> stops
        </p>
        <p>
          Shutter contribution ($-\log_2(t)$): <span className="highlight">{shutterPart.toFixed(2)}</span> stops
        </p>
        <p>
          ISO contribution ($\log_2(ISO/100)$): <span className="highlight">{isoPart.toFixed(2)}</span> stops
        </p>
        <p className="summary">
          Total EV: <strong>{ev.toFixed(2)}</strong> (Lower EV means brighter setting)
        </p>
      </div>
